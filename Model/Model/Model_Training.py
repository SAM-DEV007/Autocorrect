...
def enc_dec_model_inputs():
    '''
    Initializes placeholders for tensorflow.
    '''
...
def hyperparam_inputs():
    '''
    Initializes hyper parameters placeholders for tensorflow.
    '''
...
def process_decoder_input(incorr_data, incorr_txt_to_int, batch_size):
    '''
    Preprocess target data for encoding.
    '''
 
    go_id = incorr_txt_to_int['<GO>']
    
    after_slice = tf.strided_slice(incorr_data, [0, 0], [batch_size, -1], [1, 1])
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, target_sequence_length, max_summary_length, output_layer, keep_prob):
    '''
    Creates a training process for decoding layer.
    '''
...
def decoding_layer(dec_input, encoder_state, target_sequence_length, max_target_sequence_length, rnn_size, 
                    num_layers, target_vocab_to_int, target_vocab_size,
                    batch_size, keep_prob, decoding_embedding_size) -> tuple:
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, target_sequence_length, max_target_sentence_length,
    ...
    _, enc_states = encoding_layer(input_data, rnn_size, num_layers, keep_prob, source_vocab_size, enc_embedding_size)
...
def get_batches(sources, targets, batch_size, source_pad_int, target_pad_int):
    '''
    Provides the batches for training.

    Parameters:
    sources[list] : The list of correct text
    targets[list] : The list of incorrect text
    batch_size[int] : The size of the batch specified in the hyperparameter
    '''
...
if __name__ == '__main__':
    ...
    (INCORRECT, CORRECT), (INCORR_TXT_TO_INT, CORR_TXT_TO_INT), _ = load_pre_process()
    ...
    max_target_sentence_length = max([len(word) for word in CORRECT])
    ...
    train_source = CORRECT[batch_size:]
    train_target = INCORRECT[batch_size:]
    valid_source = CORRECT[:batch_size]
    valid_target = INCORRECT[:batch_size]
    (valid_sources_batch, valid_targets_batch, valid_sources_lengths, valid_targets_lengths ) = next(get_batches(valid_source,
                                                                                                                valid_target,
                                                                                                                batch_size,
                                                                                                                CORR_TXT_TO_INT['<PAD>'],
                                                                                                                INCORR_TXT_TO_INT['<PAD>']))                                                                                                  
            ...
            for batch_i, (source_batch, target_batch, sources_lengths, targets_lengths) in enumerate(
                ...
                if batch_i % display_step == 0 and batch_i > 0:
                    ...
                    print('Epoch {:>3} Batch {:>4}/{} - Train Accuracy: {:>6.4f}, Validation Accuracy: {:>6.4f}, Loss: {:>6.4f}'
                        .format(epoch_i, batch_i, len(CORRECT) // batch_size, train_acc, valid_acc, loss))

        # Save Model
        save_path = os.path.join(PATH, 'Trained_Model')
